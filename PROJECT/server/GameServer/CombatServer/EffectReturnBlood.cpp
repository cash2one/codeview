...
EffectReturnBlood::EffectReturnBlood()
{
}
...
bool EffectReturnBlood::Start(ICreature *pMaster, UID uidCreator, IStatus *pStatusBelong )
{
	...
	m_pMaster->SubscribeAction(msgID,this,"EffectReturnBlood::Start [enMsgID_Attacked]");

	return true;
}

void EffectReturnBlood::OnAction(XEventData & EventData)
{
	if(EventData.m_MsgID == MAKE_MSGID(CIRCULTYPE_SS,enMsgID_Attacked))
	{
		...
		case enEffectValueType_Fixed: //增减固定值
			{
				pRoundStartCnt->m_AddBloodFactor += m_pEffectCnfg->m_Value;
			}
			break;
		case enEffectValueType_Scale:         //比例(千分之)
			{
				pRoundStartCnt->m_AddBloodFactor += (float)m_pEffectCnfg->m_Value/1000;
			}
			break;
