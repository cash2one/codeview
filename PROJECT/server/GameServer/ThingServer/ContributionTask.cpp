...
ContributionTask::ContributionTask(TaskPart * pTaskPart,const STaskCnfg *  pTaskCnfg) : EventTask(pTaskPart, pTaskCnfg)
{
}


void ContributionTask::OnEvent(XEventData & EventData)
{
	...
	if(EventData.m_MsgID == msgID)
	{
		...
		SS_UseSynContribution * pUseSynContribution = (SS_UseSynContribution *)EventData.m_pContext;

		m_nCurCount += pUseSynContribution->m_UseNum;
