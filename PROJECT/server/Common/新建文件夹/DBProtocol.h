...
{
	SDB_Update_TrainingHallData_Req() : OnHookNum(0), RemainTime(0), LastFinishTime(0), GetExp(0)
	{
		Uid_User = UID().ToUint64();
		this->m_length = sizeof(*this);
	}
...
struct SDB_GiveThingMail
{
	...
	FIELD(TInt8Array<DESCRIPT_LEN_300>,		m_ContentText);		//文本内容
...
struct SDB_Get_PayData
{
	FIELD_BEGIN();
	FIELD(UINT32,		m_ID);
	FIELD(INT32,		m_Money);	//支付金额，以元为单位
...
struct SDB_RechareForward
{
	...
	FIELD(TInt8Array<DESCRIPT_LEN_300>,	m_szMailContent);	//邮件正文
