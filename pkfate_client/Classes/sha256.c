...
unsigned char *SHA_256(unsigned char *d, size_t n, unsigned char *md)
{
    sha256_context ctx;
...
char *SHA256(char *d)
{
	...
	int j;
	sha256sum=SHA_256(d,strlen(d),NULL);
	for( j = 0; j < 32; j++ )
	{
...
int main( int argc, char *argv[] )
{
    ...
    if( argc < 2 )
    {
        ...
        for( i = 0; i < 3; i++ )
        {
			...
			//SHA256(msg[i],strlen(msg[i]),sha256sum);
			sha256sum=SHA_256(msg[i],strlen(msg[i]),NULL);
